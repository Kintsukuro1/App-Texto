...
export const EmptyState = () => {
  const { createPage, getOrCreateDailyNote } = useNotesStore();
  ...
  return (
    <div className="flex-1 flex flex-col items-center justify-center p-8 text-center bg-[var(--bg-primary)] text-[var(--text-primary)]">
      <div className="max-w-sm w-full p-8 bg-[var(--bg-surface)] border border-[var(--border-muted)] rounded-3xl space-y-4 shadow-xl">
        ...
        <div className="space-y-1.5">
          ...
          <p className="text-xs text-[var(--text-secondary)] leading-relaxed">
            Crea tu primera {vocabulary.page} o abre tu nota diaria para comenzar a escribir y organizar tus ideas.
          </p>
        </div>
        ...
        <div className="space-y-2">
          ...
          <button
            ...
            className="w-full py-2 px-4 rounded-xl bg-[var(--bg-primary)] hover:bg-[var(--bg-hover)] text-[var(--text-primary)] border border-[var(--border-muted)] font-medium text-xs transition-all cursor-pointer flex items-center justify-center gap-2"
          >
            <span>📅</span>
            <span>Abrir Nota Diaria (Ctrl+D)</span>
          </button>
        </div>
      </div>
    </div>
